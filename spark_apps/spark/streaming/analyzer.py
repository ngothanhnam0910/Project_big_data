...
EXTRA_PACKAGES = "com.datastax.spark:spark-cassandra-connector_2.12:3.3.0,org.apache.spark:spark-sql-kafka-0-10_2.12:3.3.0"
...
class StreamTwitterDataTransformer():
    ...
    def transform_data(self, df):
        ...
        result_df = self.spark.read.format('org.apache.spark.sql.cassandra')\
            .options(table='stream_tweet_trending', keyspace='coinhub_2')\
            .load()
...
class StreamTwitterDataAnalyzer():
    def __init__(self):
        log_handler = RotatingFileHandler(
            f"{os.path.abspath(os.getcwd())}/apps/spark/streaming/logs/analyzer.log",
    ...
    def analyze_data(self, df, batch_id):
        print(f"Nhay vao nanalyze_data rồi ")
        ...
        df.write.format('org.apache.spark.sql.cassandra')\
          .mode('append')\
          .options(table='recent_tweet', keyspace='coinhub_2')\
          .save()
        self.transform_and_save_data(df)

    def run(self):
        #self.logger.info('Start analyzing twitter data from kafka')
        print(f"Start analyzing twitter data from kafka")
        df = self.spark.readStream.format('kafka')\
            .option('kafka.bootstrap.servers', 'host.docker.internal:29092,host.docker.internal:29093')\
