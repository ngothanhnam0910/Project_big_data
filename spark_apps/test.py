...
class CoinTradeDataTransformer():
    def __init__(self):
        ...
        #     .config("spark.app.name", "CoinTradeDataAnalyzer")\
        #     .config("spark.master", "yarn")\
        #     .config("spark.jars.packages", "com.datastax.spark:spark-cassandra-connector_2.12:3.3.4")\
        #     .config("spark.sql.extensions", "com.datastax.spark.connector.CassandraSparkExtensions")\
        #     .config("spark.cassandra.connection.host", "172.20.0.11")\
        #     .getOrCreate()
...
if __name__ == "__main__":
    coin_trade = CoinTradeDataTransformer()
    file = "/opt/spark/data/coinTradeData_2023-12-18_13-59-35.csv"
    coin_trade.transform_and_save_data(file)
    # print(result["minute"])
    print(f"----------------------thanh cong -------------")
    # print(output["minute"].show())
